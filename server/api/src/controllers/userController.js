...
function verifyJWT(token){
  const tokenVerify = jwt.decode(token, {complete: true});
  return tokenVerify;
}    
...
server.put("/alterar-senha", async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case !senha || !senha.trim() || senha.length > 50:
        throw new Error("A Senha é obrigatório");
      default:
        break;
    }
    senha = sha256(senha);
    const answer = await userAlterarPassword(senha, auth.id);
...
server.delete("/usuario", async (req, res) => {
  try {
    const header = req.header("x-access-token");
    const auth = jwt.decode(header);
    if (!header || !auth || !(await userIdSearch(auth.id)))
      throw new Error("Falha na autenticação");
