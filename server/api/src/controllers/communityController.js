...
// Enviar imagem
server.put(
    "/comunidade/imagem/:id",
    communityImg.single("imagem"),
    async (req, res) => {
        try {
            const header = req.header("x-access-token");
...
server.put("/comunidade/:id", async (req, res) => {
    try {
        ...
        switch (true) {
            case !header ||
                !auth ||
                !(await communityOwner(auth.id, community.id)):
        ...
        if (r < 1)
            throw new Error(
                "Não foi possível fazer as alterações na comunidade"
            );
...
server.put("/comunidade/admin/usuario", async (req, res) => {
    try {
        ...
        switch (true) {
            ...
            case !header ||
                !auth ||
                !(await userIdSearch(auth.id)) ||
                !(await communityOwner(auth.id, comunidade)):
                throw new Error("Falha na autenticação");
            case !id ||
                !situacao ||
                !comunidade ||
                !["A", "N"].includes(situacao):
...
server.get("/comunidade/usuario", async (req, res) => {
    try {
        const user = req.body;
        if (!user.comunidade || !communityId(user.comunidade))
